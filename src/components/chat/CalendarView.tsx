...
import { Calendar as CalendarIcon, Clock, Plus, Video, ChevronLeft, ChevronRight, MoreHorizontal, Trash2, Edit2, X } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';

export const CalendarView = () => {
  const [currentDate, setCurrentDate] = useState(new Date(2026, 3, 1)); // April 2026 default
  const [selectedDate, setSelectedDate] = useState<number>(12); // Default to the 12th
  const [view, setView] = useState<'month' | 'week'>('month');

  // Mutable state for events
  const [events, setEvents] = useState<Record<number, any[]>>({
    12: [
      { id: 1, title: 'Weekly Sync', time: '10:00 AM - 11:30 AM', type: 'video', members: 3 },
      { id: 2, title: 'Design Review', time: '2:00 PM - 3:00 PM', type: 'meeting', members: 5 }
    ],
    14: [
      { id: 3, title: 'Client Feedback', time: '9:00 AM - 9:45 AM', type: 'video', members: 2 }
    ],
    22: [
      { id: 4, title: 'Product Launch Prep', time: '1:00 PM - 4:00 PM', type: 'meeting', members: 8 }
    ]
  });
  ...
  const handleDelete = (day: number, id: number) => {
    setEvents(prev => ({
      ...prev,
      [day]: prev[day].filter(ev => ev.id !== id)
    }));
  };
  ...
  const handleSaveEvent = () => {
    ...
    if (editingEvent) {
      ...
      setEvents(prev => {
        ...
        return {
          ...prev,
          [selectedDate]: dayEvents.map(ev => ev.id === editingEvent.id ? { ...ev, ...eventForm } : ev)
        };
      });
    } else {
      // Create new
  ...
  const currentEvents = events[selectedDate] || [];

  return (
    <div className="flex-1 h-full bg-[#0F0F12] flex flex-col transition-colors duration-300 overflow-hidden text-white relative">
      
      {/* Header */}
      <div className="px-6 md:px-10 py-8 md:py-12 flex flex-col md:flex-row justify-between items-start md:items-end gap-4 border-b border-outline-variant z-10 bg-[#0F0F12]/80 backdrop-blur-md">
        <div>
          <h1 className="text-3xl md:text-4xl font-black font-display tracking-tight text-white mb-2">Schedule</h1>
      ...
      <div className="flex-1 p-6 md:p-10 flex flex-col xl:flex-row gap-8 overflow-y-auto no-scrollbar">
         ...
         <div className="w-full xl:w-[35%] flex flex-col gap-6 shrink-0 lg:max-h-full">
            <div className="flex justify-between items-center">
              ...
              <span className="text-[10px] font-bold bg-[#161618] px-3 py-1 rounded-full border border-outline-variant">{currentEvents.length} Events</span>
            </div>
            ...
            <AnimatePresence mode="wait">
              {currentEvents.length > 0 ? (
                ...
                >
                  {currentEvents.map(event => (
                    <motion.div 
                      ...
                      className="p-6 bg-[#161618] border border-outline-variant rounded-[2rem] flex flex-col gap-4 group/event transition-colors shadow-sm relative overflow-hidden"
                    >
                       <div className="flex justify-between items-start">
                          <div className={`w-12 h-12 rounded-[1.2rem] flex items-center justify-center ${event.type === 'video' ? 'bg-noir-accent/20 text-noir-accent' : 'bg-purple-500/20 text-purple-400'}`}>
                             {event.type === 'video' ? <Video size={20} /> : <CalendarIcon size={20} />}
                          </div>
                          
                          {/* Actions (Edit / Delete) */}
                          <div className="flex gap-2 opacity-100 xl:opacity-0 group-hover/event:opacity-100 transition-opacity bg-[#0F0F12] p-1 rounded-xl border border-outline-variant">
                             <button onClick={() => handleOpenModal(event)} className="p-2 text-text-muted hover:text-noir-accent transition-colors">
                               <Edit2 size={14} />
                             </button>
                             <button onClick={() => handleDelete(selectedDate, event.id)} className="p-2 text-text-muted hover:text-red-400 transition-colors">
                               <Trash2 size={14} />
                             </button>
                          </div>
                       </div>
                ...
                <motion.div 
                  ...
                  animate={{ opacity: 1 }}
                  exit={{ opacity: 0 }}
                  className="flex-1 min-h-[250px] flex flex-col items-center justify-center border-2 border-dashed border-outline-variant rounded-[2rem] bg-[#161618]/30 p-10"
                >
         ...
         <div className="flex-1 bg-[#161618] rounded-[2.5rem] border border-outline-variant p-6 md:p-8 flex flex-col shadow-xl min-w-0">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
               <div className="flex items-center gap-4 w-full md:w-auto justify-between">
                 ...
                 <div className="flex gap-1 bg-[#242426] rounded-[1rem] p-1 border border-outline-variant">
                   <button onClick={prevMonth} className="px-3 py-2 hover:bg-[#333336] rounded-lg transition-colors"><ChevronLeft size={16} /></button>
                   <button onClick={nextMonth} className="px-3 py-2 hover:bg-[#333336] rounded-lg transition-colors"><ChevronRight size={16} /></button>
                 </div>
               </div>
               <div className="flex gap-2 w-full md:w-auto bg-[#0F0F12] p-1 rounded-[1.4rem]">
                  <button 
                    onClick={() => setView('month')}
                    className={`flex-1 md:flex-none px-5 py-2.5 rounded-[1.2rem] text-xs font-bold transition-all ${view === 'month' ? 'bg-[#242426] text-white shadow-sm border border-outline-variant' : 'bg-transparent text-text-muted hover:text-white'}`}
                  >
                    Month
                  </button>
                  <button 
                    onClick={() => setView('week')}
                    className={`flex-1 md:flex-none px-5 py-2.5 rounded-[1.2rem] text-xs font-bold transition-all ${view === 'week' ? 'bg-[#242426] text-white shadow-sm border border-outline-variant' : 'bg-transparent text-text-muted hover:text-white'}`}
                  >
                    Week
                  </button>
               </div>
            ...
            <div className="flex-1 flex flex-col overflow-x-auto no-scrollbar">
               <div className="min-w-[500px]">
                 ...
                 <div className="flex-1 grid grid-cols-7 gap-2 md:gap-3 grid-rows-5">
                    {/* Empty offsets */}
                        ...
                        <motion.div 
                           ...
                           className={`rounded-[1rem] md:rounded-[1.5rem] border p-2 md:p-3 flex flex-col items-center justify-center transition-colors cursor-pointer relative min-h-[60px] md:min-h-[80px]
                              ${isSelected 
                                ...
                                : 'border-outline-variant/30 hover:border-outline-variant bg-[#0F0F12]/50'
                              }`}
                        >
                           <span className={`text-sm md:text-base font-bold ${isSelected ? 'text-white' : 'text-text-muted md:group-hover:text-white'}`}>
                              {day}
                           </span>
                           ...
                           {hasEvents && (
                             <div className="absolute bottom-1.5 md:bottom-2 flex gap-1 flex-wrap justify-center px-1">
                               {events[day].map((ev, idx) => (
                                 <div key={idx} className={`w-1.5 h-1.5 md:w-2 md:h-2 rounded-full ${isSelected ? 'bg-white' : (ev.type === 'video' ? 'bg-noir-accent' : 'bg-purple-500')}`} />
                               ))}
                             </div>
                           )}
                        </motion.div>
        ...
        {isModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div 
              ...
              className="relative w-full max-w-[400px] bg-[#161618] rounded-[2.5rem] border border-outline-variant p-8 shadow-2xl"
            >
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-xl font-black font-display text-white tracking-tight">{editingEvent ? 'Edit Event' : 'New Event'}</h3>
                <button onClick={() => setIsModalOpen(false)} className="p-2 text-text-muted hover:text-white bg-[#0F0F12] rounded-xl transition-colors shrink-0">
                  <X size={18} />
                </button>
              </div>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input 
                    ...
                    className="w-full bg-[#0F0F12] border border-outline-variant rounded-[1rem] px-4 py-3 text-sm font-bold text-white focus:outline-none focus:border-noir-accent transition-colors"
                ...
                <div className="flex gap-4">
                  <div className="flex-1">
                    ...
                    <select 
                      ...
                      className="w-full bg-[#0F0F12] border border-outline-variant rounded-[1rem] px-4 py-3 text-sm font-bold text-white focus:outline-none focus:border-noir-accent transition-colors appearance-none"
                    >
